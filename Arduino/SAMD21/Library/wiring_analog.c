...
bool startAnalogRead(uint32_t pin)
{
...
bool fastAnalogRead(uint32_t nextRead, uint32_t* valueRead )
{
  uint32_t tmp;
  ...
  tmp = ADC->RESULT.reg;
  *valueRead = mapResolution(tmp, _ADCResolution, _readResolution);
  
  if( nextRead )
  {
	syncADC();
    ADC->INPUTCTRL.bit.MUXPOS = g_APinDescription[nextRead].ulADCChannelNumber; // Selection for the positive ADC input
...
{
  uint32_t valueRead = 0;
  if( startAnalogRead(pin))
  {
    while( fastAnalogRead( 0, &valueRead ) == false );
