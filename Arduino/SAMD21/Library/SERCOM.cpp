...
uint8_t UARTidxIn = 0;
uint8_t UARTidxOut = 0;

int SERCOM::writeDataUART(uint8_t data)
{
  // If data register is empty
  while( !isDataRegisterEmptyUART() || UARTidxIn != UARTidxOut );
...
void SERCOM::writeDataUART_noWait(const uint8_t* data, uint8_t size )
{
  // Add data to circular buffer
  while( size )
  {
    UARTBuffer[UARTidxIn] = *data;
	data++;
	size--;
    UARTidxIn++;
    if( UARTidxIn >= MAX_UART_BUFFER ) UARTidxIn = 0;
  }
  
  // If data register is empty and there is data to be sent
  if( UARTidxIn != UARTidxOut && isDataRegisterEmptyUART())
  {  
	  //Put data into DATA register
	  sercom->USART.DATA.reg = (uint16_t)UARTBuffer[UARTidxOut];
	  UARTidxOut++;
	  if( UARTidxOut >= MAX_UART_BUFFER ) UARTidxOut = 0;
  }
}
...
typedef enum {
	SERCOM_IDLE,
	SERCOM_WAITING_FOR_ADDRESS_TX,
	SERCOM_WAITING_FOR_DATA_TX,
	SERCOM_WAITING_FOR_SYNC,
} SERCOM_ASYNC_STATE_T;

SERCOM_ASYNC_STATE_T pendingState = SERCOM_IDLE;

#define MAX_I2C_BUFFER_SIZE		128
uint8_t pendingData[MAX_I2C_BUFFER_SIZE];
uint8_t pendingIdxIn = 0;
uint8_t pendingIdxOut = 0;

int SERCOM::sendDataMasterWIRE_noWait(uint8_t address, uint8_t* data, uint8_t size)
{
  while( size )
  {
    pendingData[pendingIdxIn] = *data;
    pendingIdxIn++;
	data++;
	size--;
	if( pendingIdxIn >= MAX_I2C_BUFFER_SIZE ) pendingIdxIn = 0;
  }
  
  switch( pendingState )
  {
	case SERCOM_IDLE :
	  if( pendingIdxIn != pendingIdxOut )
      {
		// 7-bits address + 1-bits R/W
	    address = (address << 0x1ul) | WIRE_WRITE_FLAG;
        // Send start and address
        sercom->I2CM.ADDR.bit.ADDR = address;
	    pendingState = SERCOM_WAITING_FOR_ADDRESS_TX;
	  }
	  else return 0; // Indicates that all data has been sent
	  break;
	  
	case SERCOM_WAITING_FOR_ADDRESS_TX :
	  // Wait transmission complete
	  if( sercom->I2CM.INTFLAG.bit.MB )
      {
		/*
		...
		*/
        //Send data
        sercom->I2CM.DATA.bit.DATA = pendingData[pendingIdxOut]; 
		pendingState = SERCOM_WAITING_FOR_DATA_TX;		
      }
	  break;
	  
	case SERCOM_WAITING_FOR_DATA_TX :
      //Wait transmission successful
      if( sercom->I2CM.INTFLAG.bit.MB )
	  {
		pendingIdxOut++;
		if( pendingIdxOut >= MAX_I2C_BUFFER_SIZE ) pendingIdxOut = 0;
		if( pendingIdxOut == pendingIdxIn )
		{		  
		  sercom->I2CM.CTRLB.bit.CMD = WIRE_MASTER_ACT_STOP;
		  pendingState = SERCOM_WAITING_FOR_SYNC;
		  return 1; // Indicates that we're still wairting for SYNC
		}
		else
		{
		  pendingState = SERCOM_WAITING_FOR_ADDRESS_TX;
	  ...
	  else
	  {
		...
		if (sercom->I2CM.STATUS.bit.BUSERR)
		{
		  pendingState = SERCOM_IDLE;
		}
	  }
	  break;
	  
	case SERCOM_WAITING_FOR_SYNC :
	  if( sercom->I2CM.SYNCBUSY.bit.SYSOP == 0 )
	  {
		  // Done!
		  pendingState = SERCOM_IDLE;
	  }
	  else return 1; // Still waiting on the last byte SYNC
	  break;
  ...
  if( pendingIdxIn >= pendingIdxOut ) 
	  return pendingIdxIn - pendingIdxOut;
	else
	  return pendingIdxIn - pendingIdxOut + MAX_I2C_BUFFER_SIZE;
}
