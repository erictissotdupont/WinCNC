...
unsigned long g_missedStepCount = 0;
  ...
  esp_timer_handle_t g_motorTimer; // Timer for the step
...
void Motor::Init( )
{
  ...
  timer.callback = pulse_timer_callback;
  timer.arg = this;
  timer.dispatch_method = ESP_TIMER_ISR; // ESP_TIMER_TASK; // ESP_TIMER_ISR. Available only if CONFIG_ESP_TIMER_SUPPORTS_ISR_DISPATCH_METHOD is enabled (by default disabled).
  timer.name = "timeout";
  timer.skip_unhandled_events = false;    
  ESP_ERROR_CHECK(esp_timer_create(&timer, &pulseTimer));
}
...
void Motor::Step( )
{
  ...
  esp_timer_start_once( pulseTimer, 100 );
...
static void timer_callback(void* arg)
{
    Y.Step( );
    nextTime += 2000;
    esp_timer_stop(g_motorTimer);
    //esp_timer_start_once( g_motorTimer, 2000 ); //nextTime - esp_timer_get_time( ));
    esp_timer_start_once( g_motorTimer, nextTime - esp_timer_get_time( ));
}
...
{      
  ...
  timer.name = "timeout";
  timer.skip_unhandled_events = false;    
  ESP_ERROR_CHECK(esp_timer_create(&timer, &g_motorTimer));
  ...
  nextTime = esp_timer_get_time( ) + 2000;
  ESP_ERROR_CHECK(esp_timer_start_once( g_motorTimer, 2000 ));
