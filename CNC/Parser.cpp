	...
	LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		else
		{
			...
			pJob->hDialog = hWnd;
			pJob->hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ParserThread, pJob, 0, &dwThread);
			return (pJob->hThread != NULL);
		}
		break;
...
WCHAR* GetCNCErrorString(tStatus status)
{
	...
	case retFileReadFailed: return L"File read error";
...
tStatus ParseBuffer( HWND hParent, char* pt, ULONG cbBuffer, tStatus(*cmd)(char*), BOOL bDebug )
{
	...
	// Wait for thread to terminate (10 sec timeout)
	if (job.hThread)
	{
		WaitForSingleObject(job.hThread, 10000);
		CloseHandle(job.hThread);
	}

	// Wait for the window to be closed (10 sec timeout)
	int timeout = 100;
	while (job.hDialog && --timeout >= 0 ) Sleep(100);
...
tStatus ParseGCodeFile( HWND hParent, LPWSTR szFileName, tStatus(*cmd)(char*), BOOL bDebug )
{
	...
	if (ReadFile(hFile, buffer, fileSize, NULL, NULL) == FALSE)
	{
		ret = retFileReadFailed;
	}
	else
	{
		buffer[fileSize] = 0;
